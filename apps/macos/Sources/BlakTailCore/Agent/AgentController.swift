...
public struct AgentController: Sendable {
    ...
    /// Arguments for `blaktaild up`. The join key is intentionally absent.
    public static func upArguments(
        ...
        [agentPath, "up", "--coordinator", coordinator, "--name", name, "--address", address]
    }
    ...
    public func connect(joinKey: String, coordinator: String, name: String, address: String) throws {
        guard FileManager.default.isExecutableFile(atPath: agentPath) else {
