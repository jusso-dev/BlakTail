...
async fn register_node(
    ...
    if input.name.trim().is_empty()
        ...
        || input
            .endpoint
            .as_ref()
            .is_some_and(|x| x.parse::<SocketAddr>().is_err())
    {
        return Err(ApiError::BadRequest(
    ...
    tx.execute("INSERT INTO nodes(id,org_id,name,wg_public_key,allowed_ips_json,endpoint,token_hash,created_at) VALUES(?1,?2,?3,?4,?5,?6,?7,?8)",params![id.to_string(),org_id,input.name.trim(),input.wg_public_key.trim(),serde_json::to_string(&input.allowed_ips).unwrap(),input.endpoint,hash(&token),now()]).map_err(conflict("node name or public key already exists in org"))?;
    if single {
        tx.execute(
...
async fn list_peers(
    ...
    let mut q=db.prepare("SELECT id,name,wg_public_key,allowed_ips_json,endpoint FROM nodes WHERE org_id=?1 AND id!=?2 AND revoked_at IS NULL ORDER BY name")?;
    let peers = q
...
mod tests {
    ...
    async fn two_nodes_and_revocation() {
        ...
        for n in 1..=2 {
            ...
            .await;
            let x=call(&r,Method::POST,"/v1/nodes/register",serde_json::json!({"join_key":k.key,"name":format!("node-{n}"),"wg_public_key":format!("key-{n}"),"allowed_ips":[format!("100.64.0.{n}/32")],"endpoint":format!("192.0.2.{n}:51820")}),None).await;
