...
async fn register_node(
    ...
    tx.execute("INSERT INTO nodes(id,org_id,name,wg_public_key,endpoint,allowed_ips_json,token_hash,created_at) VALUES(?1,?2,?3,?4,?5,?6,?7,?8)",params![id.to_string(),org_id,input.name.trim(),input.wg_public_key.trim(),input.endpoint,serde_json::to_string(&allowed_ips).unwrap(),hash(&token),now()]).map_err(conflict("node name, public key, or address already exists in org"))?;
    if single {
        tx.execute(
...
async fn list_peers(
    ...
    let mut q=db.prepare("SELECT id,name,wg_public_key,endpoint,allowed_ips_json FROM nodes WHERE org_id=?1 AND id!=?2 AND revoked_at IS NULL ORDER BY name")?;
    let peers = q
